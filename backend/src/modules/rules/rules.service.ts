...
export class RulesService {
    ...
    async get(rule_id: string) {
        return await this.db.select().from(schema.rules).where(eq(schema.rules.id, rule_id));
    }

    async update(rule_id: string, rule: Rule) {
        await this.db.update(schema.rules).set(rule).where(eq(schema.rules.id, rule_id));
    }
