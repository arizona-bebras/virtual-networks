import { integer, pgTable, varchar, text, jsonb, uuid, primaryKey } from 'drizzle-orm/pg-core';
...
export const networks = pgTable('networks', {
  ...
  config: text().notNull(),
  admin_id: uuid().notNull().references(() => users.id),
});
...
export const devices = pgTable('devices', {
  ...
  config: text().notNull(),
  network_id: uuid().notNull().references(() => networks.id),
});
...
export const relations = defineRelations({ users, networks, devices, tags, devicesTags }, (r) => ({
  users: {
